...
struct Any {
  ...
  void push_duktape(duk_context *ctx) const { ptr->push_to_duktape(ctx); }

  template <class U> operator U() { return as<StorageType<U>>(); }
...
private:
  ...
  template <typename T> struct Derived : Base {
    ...
    void push_to_duktape(duk_context *ctx) const { to_duktape(ctx, value); }
  };
